...
bool QuadrotorUKF::ProcessUpdate(Eigen::Matrix<double, Eigen::Dynamic, 1> u, ros::Time time)
{
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 1> x = ProcessModel(xHist_.front(), u, Eigen::MatrixXd::Zero(procNoiseCnt_,1), dt);//ProcessModel(xHist.front(), u, zeros<colvec>(procNoiseCnt), dt);
  xHist_.push_front(x);
  uHist_.push_front(u);
  xTimeHist_.push_front(time);
...
bool QuadrotorUKF::MeasurementUpdateSLAM(const Eigen::Matrix<double, Eigen::Dynamic, 1>& z, const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>& RnSLAM, ros::Time time)
{
  ...
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> K = P_ * H.transpose() * S.inverse();
  // Innovation
